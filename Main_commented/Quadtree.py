class Quadtree(object): #Creating the class of objects called Quadtree
    ...
    def __init__(self, siz=1000): #Function that defines the initial conditions of the tree: the size of the canvas over which it will act.
        self.siz = siz
        self.structure = Node(0,0,self.siz) # Structure is the original node, containing all the subnodes
    ...
    def buildTree(self,stars): #Defining the function that will build the original tree. It takes the elements, "stars", as its input.
        self.structure = Node(0,0,self.siz) #Defines the original structure of the tree as being at point 0,0, and having the size as defined above.
        for i in stars:
            self.structure.insertStar(i) #Iterates over every star in the list of stars, inserting the star as defined in the function below.
    ...
    def insertStar(self,star): #Function that decides whether to insert the star in the node, whether to continue dividing the node.
            if self.isInNode(star.pos):  #First check star is in node.
                if not self.containsStar: #If there are currently no other stars in the node, star is inserted.
                    ...
                    for k in self.stars: # Iterate through the stars withis the (main) node
                        for l in self.subnodes: #Iterate through the subnodes                
                            l.insertStar(k) #Inserts star into one of the subnodes of the node. Repeats the process for this subnode. Note this is a recursive function.
